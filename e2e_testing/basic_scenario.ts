...
const main = async () => {
  ...
  await factoryClient
    .createPair(
      {
        assetInfos: [{ native_token: { denom: "ufoo" } }, { native_token: { denom: "uluna" } }],
        pairType: { xyk: {} },
      },
      "auto",
    )
  ...
  await factoryClient
    .createPair(
      {
        assetInfos: [{ native_token: { denom: "uluna" } }, { native_token: { denom: "ufoo" } }],
  ...
  await factoryClient
    .createPair(
      {
        assetInfos: [{ native_token: { denom: "utia" } }, { native_token: { denom: "usttia" } }],
        pairType: { concentrated: {} },
  ...
  await incentivesClient
    .incentivize(
      ...
      [
        {
          denom: "uluna",
          amount: (10_000_000000 + Number.parseInt(config.incentivize_fee.fee.amount)).toString(),
        },
      ],
    )
    .then(console.log);
