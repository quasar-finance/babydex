...
const main = async () => {
  ...
  const token0Denom =
    "native_token" in poolState.assets[0].info ? poolState.assets[0].info.native_token.denom : "";
  const token1Denom =
    "native_token" in poolState.assets[1].info ? poolState.assets[1].info.native_token.denom : "";
  ...
  const coins: Coin[] = assets.map((asset) => coin(asset.amount, asset.info.native_token.denom));
