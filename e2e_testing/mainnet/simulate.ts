...
const main = async () => {
  ...
  const priceScale = Number.parseFloat(poolParams.price_scale);
  console.log(`Pool price scale: ${priceScale}`);

  // Function to calculate optimal amounts using only price scale
  function calculateOptimalAmountsSimple(
    amount: number,
    isAsset0: boolean,
  ): { asset0: number; asset1: number } {
    if (isAsset0) {
  ...
  for (let i = 20; i <= 40; i++) {
    ...
    try {
      ...
      if (result) {
        ...
        console.log(
          `Withdraw result for [USDC: ${usdcAmount}, BBN: ${bbnAmount}]:`,
          withdrawResult,
        );
      }
    } catch (error: any) {
