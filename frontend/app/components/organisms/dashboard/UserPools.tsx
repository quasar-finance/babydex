...
export const UserPools: React.FC<Props> = ({ pools, isLoading, refreshUserPools }) => {
  ...
  const filteredPools = pools.filter(({ poolInfo }) => poolInfo.name.toLowerCase().includes(searchText.toLowerCase()));
  ...
  const poolAddresses = useMemo(() => filteredPools.map(({ poolInfo }) => poolInfo.poolAddress), [filteredPools]);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Table columns={columns} gridClass={gridClass}>
        ...
        {filteredPools
          .sort((a, b) => (BigInt(a.userBalance.staked_share_amount) < BigInt(b.userBalance.staked_share_amount) ? 1 : -1))
          .map(({ poolInfo, userBalance, incentives }, i) => {
            return (
              <TableRow key={i} gridClass={twMerge('grid cursor-pointer hover:bg-white/5', gridClass)} onClick={() => router.push(`/pools/${poolInfo.poolAddress}`)}>
                <CellPoolName
                ...
                />
                <CellDataToken title="Staked" poolAddress={poolInfo.poolAddress} amount={userBalance.staked_share_amount} tokens={poolInfo.assets} className="w-full pl-4" />
                <CellApr
                ...
                <CellVolume
                  ...
                  className="w-full px-4"
                />
                <CellClaimRewards rewards={incentives} poolToken={userBalance.lpToken} stakedAmount={userBalance.staked_share_amount} className="w-full px-4" />
                <div
