...
export function multisig(parameters: InjectedParameters = {}) {
  ...
  return createConnector<Provider | undefined, Properties, StorageItem>((config: any) => ({
    get icon() {
    ...
    async connect({ chainId: chain, isReconnecting } = {}) {
      ...
      if (!isReconnecting) config.storage?.setItem("injected.chainId", chainId.toString());

      try {
        if (!accounts?.length && !isReconnecting) {
          await provider.enable(chainId.toString()).catch(async (err: Error) => {
            ...
            if (err.message === "Invalid chain id") {
              ...
              if (!registry) throw new Error("Chain registry is required to suggest chain");
              const { assets, chain } = registry;

              const [chainRes, assetsRes] = await Promise.all([fetch(chain), fetch(assets)]);
            } else throw err;
          });
        }
    ...
    async getProvider() {
      ...
      if (typeof target.provider === "function") provider = target.provider();
    ...
    async getChainId() {
      return (await config.storage?.getItem("injected.chainId")) as unknown as number;
    },
    async onDisconnect() {
