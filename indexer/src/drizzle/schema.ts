...
export const stakeLiquidityInV1Cosmos = v1Cosmos.view("stake_liquidity", {	sender: text(),
	...
	transactionIndex: integer("transaction_index"),
	data: jsonb(),
}).as(sql`SELECT attributes(events.*) ->> 'sender'::text AS sender, attributes(events.*) ->> 'user'::text AS owner, (attributes(events.*) ->> 'amount'::text)::numeric AS amount, attributes(events.*) ->> 'lp_token'::text AS lp_token, attributes(events.*) ->> '_contract_address'::text AS pool_address, (attributes(events.*) ->> 'msg_index'::text)::integer AS msg_index, events.chain_id AS internal_chain_id, events.block_hash, events.height, events.index, events."time" AS "timestamp", events.transaction_hash, events.transaction_index, events.data FROM v1_cosmos.events WHERE (events.data ->> 'type'::text) = 'wasm-deposit'::text`);
...
}).as(sql`WITH combined_liquidity_operations AS ( SELECT CASE WHEN add_liquidity.receiver IS NOT NULL THEN add_liquidity.receiver ELSE add_liquidity.sender END AS owner, add_liquidity.pool_address, add_liquidity.share AS staked_share_amount, add_liquidity."timestamp" FROM v1_cosmos.add_liquidity UNION ALL SELECT withdraw_liquidity.sender AS owner, withdraw_liquidity.pool_address, - withdraw_liquidity.share AS staked_share_amount, withdraw_liquidity."timestamp" FROM v1_cosmos.withdraw_liquidity ) SELECT combined_liquidity_operations.pool_address, combined_liquidity_operations.owner, sum(combined_liquidity_operations.staked_share_amount) AS staked_share_amount, max(combined_liquidity_operations."timestamp") AS last_update_time FROM combined_liquidity_operations GROUP BY combined_liquidity_operations.pool_address, combined_liquidity_operations.owner HAVING sum(combined_liquidity_operations.staked_share_amount) > 0::numeric ORDER BY combined_liquidity_operations.pool_address, combined_liquidity_operations.owner`);
