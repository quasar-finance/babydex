...
export const poolLpTokensIdSeqInV1Cosmos = v1Cosmos.sequence("pool_lp_tokens_id_seq", {
  startWith: "1",
  increment: "1",
  minValue: "1",
...
export const contractStatusInHubble = hubble.table(
  "contract_status",
  {
...
export const clientsInHubble = hubble.table(
  "clients",
  {
...
export const blockStatusInHubble = hubble.table(
  "block_status",
  {
...
export const consensusHeightsInHubble = hubble.table(
  "consensus_heights",
  {
    chainId: integer("chain_id").notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const chainsInHubble = hubble.table(
  "chains",
  {
    id: serial().notNull(),
  ...
  (table) => [
    index("chains_chain_id_idx").using(
      "btree",
      table.chainId.asc().nullsLast().op("text_ops"),
...
export const assetsInHubble = hubble.table(
  "assets",
  {
    chainId: integer("chain_id").notNull(),
    denom: text().notNull(),
...
export const eventsInV1Cosmos = v1Cosmos.table(
  "events",
  {
    chainId: integer("chain_id").notNull(),
  ...
  (table) => [
    ...
    index("events_recv_packet_by_chain_destination_channel_sequence_idx")
      .using(
        "btree",
        sql`chain_id`,
        sql`((attributes(events.*) ->> 'packet_sequence'::text))::numeric`,
        sql`null`,
      )
      .where(sql`((data ->> 'type'::text) = 'recv_packet'::text)`),
    index("events_send_packet_by_chain_id_tx_hash_msg_index_idx")
      .using("btree", sql`chain_id`, sql`transaction_hash`, sql`null`)
      .where(sql`((data ->> 'type'::text) = 'send_packet'::text)`),
    index("events_send_packet_by_time_idx")
      .using("btree", table.time.desc().nullsFirst().op("timestamptz_ops"))
      .where(sql`((data ->> 'type'::text) = 'send_packet'::text)`),
    index("events_send_packet_by_tx_hash_msg_index_idx")
      .using("btree", sql`transaction_hash`, sql`null`)
      .where(sql`((data ->> 'type'::text) = 'send_packet'::text)`),
    index("events_transaction_hash_int4_idx").using("btree", sql`transaction_hash`, sql`null`),
    index("events_update_client_by_chain_id_revision_height_idx")
      .using("btree", sql`chain_id`, sql`'-'::text`)
      .where(sql`((data ->> 'type'::text) = 'update_client'::text)`),
    index("events_wasm_ibc_transfer_by_time_idx")
      .using("btree", table.time.desc().nullsFirst().op("timestamptz_ops"))
      .where(
        sql`(((data ->> 'type'::text) = 'wasm-ibc_transfer'::text) AND ((attributes(events.*) ->> 'assets'::text) IS NOT NULL))`,
      ),
    index("idx_events_height").using(
      "btree",
      table.chainId.asc().nullsLast().op("int8_ops"),
      table.height.asc().nullsLast().op("int4_ops"),
    ),
    index("idx_events_height_desc").using(
      "btree",
      table.chainId.asc().nullsLast().op("int4_ops"),
      table.height.desc().nullsFirst().op("int8_ops"),
    ),
...
export const contractsInV1Cosmos = v1Cosmos.table(
  "contracts",
  {
    internalChainId: integer("internal_chain_id").notNull(),
    address: text().notNull(),
    ...
    endHeight: bigint("end_height", { mode: "number" })
      .default(sql`'9223372036854775807'`)
      .notNull(),
    description: text(),
    createdAt: timestamp("created_at", { withTimezone: true, mode: "string" })
      .defaultNow()
...
export const tokenInV1Cosmos = v1Cosmos.table(
  "token",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: "number" }).generatedByDefaultAsIdentity({
      name: "v1_cosmos.token_id_seq",
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: 9223372036854775807,
...
export const transactionsInV1Cosmos = v1Cosmos.table(
  "transactions",
  {
    chainId: integer("chain_id").notNull(),
    blockHash: text("block_hash").notNull(),
  ...
  (table) => [
    index("transactions_chain_id_height").using(
      "btree",
      table.chainId.asc().nullsLast().op("int4_ops"),
      table.height.desc().nullsFirst().op("int8_ops"),
    ),
...
export const blocksInV1Cosmos = v1Cosmos.table(
  "blocks",
  {
    chainId: integer("chain_id").notNull(),
    hash: text().notNull(),
...
export const poolLpTokenInV1Cosmos = v1Cosmos.table(
  "pool_lp_token",
  {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    id: bigint({ mode: "number" }).generatedByDefaultAsIdentity({
      name: "v1_cosmos.pool_lp_token_id_seq",
      startWith: 1,
      increment: 1,
      minValue: 1,
      maxValue: 9223372036854775807,
  ...
  (table) => [
    index("idx_pool_lp_token_lp_token").using(
      "btree",
      table.lpToken.asc().nullsLast().op("text_ops"),
    ),
  ],
);
...
export const tokenPricesInV1Cosmos = v1Cosmos.table("token_prices", {
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  id: bigint({ mode: "number" }).generatedByDefaultAsIdentity({
    name: "v1_cosmos.token_prices_id_seq",
    startWith: 1,
    increment: 1,
    minValue: 1,
    maxValue: 9223372036854775807,
...
export const materializedHistoricPoolYieldInV1Cosmos = v1Cosmos
  .materializedView("materialized_historic_pool_yield", {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const poolsInV1Cosmos = v1Cosmos
  .view("pools", {
    token0: text(),
    token1: text(),
    poolAddress: text("pool_address"),
...
export const withdrawLiquidityInV1Cosmos = v1Cosmos
  ...
  .as(
    sql`SELECT attributes(events.*) ->> 'sender'::text AS sender, COALESCE(attributes(events.*) ->> 'receiver'::text, attributes(events.*) ->> 'sender'::text) AS receiver, regexp_replace(split_part(attributes(events.*) ->> 'refund_assets'::text, ', '::text, 1), '^\d+'::text, ''::text) AS token0_denom, (regexp_matches(attributes(events.*) ->> 'refund_assets'::text, '^\d+'::text))[1]::numeric AS token0_amount, regexp_replace(split_part(attributes(events.*) ->> 'refund_assets'::text, ', '::text, 2), '^\d+'::text, ''::text) AS token1_denom, (regexp_matches(split_part(attributes(events.*) ->> 'refund_assets'::text, ', '::text, 2), '^\d+'::text))[1]::numeric AS token1_amount, (attributes(events.*) ->> 'withdrawn_share'::text)::numeric AS share, ( SELECT plt.pool FROM v1_cosmos.pool_lp_token plt WHERE plt.pool = (attributes(events.*) ->> '_contract_address'::text) OR plt.lp_token = (attributes(events.*) ->> '_contract_address'::text) LIMIT 1) AS pool_address, (attributes(events.*) ->> 'msg_index'::text)::integer AS msg_index, events.chain_id AS internal_chain_id, events.block_hash, events.height, events.index, events."time" AS "timestamp", events.transaction_hash, events.transaction_index, events.data FROM v1_cosmos.events WHERE (events.data ->> 'type'::text) = 'wasm-withdraw_liquidity'::text`,
  );

export const historicPoolYieldInV1Cosmos = v1Cosmos
  .view("historic_pool_yield", {
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    height: bigint({ mode: "number" }),
    timestamp: timestamp({ withTimezone: true, mode: "string" }),
    poolAddress: text("pool_address"),
...
export const materializedSwapInV1Cosmos = v1Cosmos
  .materializedView("materialized_swap", {
    sender: text(),
    receiver: text(),
    askAsset: text("ask_asset"),
    commissionAmount: numeric("commission_amount"),
...
export const materializedPoolsInV1Cosmos = v1Cosmos
  .materializedView("materialized_pools", {
    token0: text(),
    token1: text(),
    poolAddress: text("pool_address"),
    msgIndex: integer("msg_index"),
...
export const materializedIncentivizeInV1Cosmos = v1Cosmos
  .materializedView("materialized_incentivize", {
    lpToken: text("lp_token"),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    startTs: bigint("start_ts", { mode: "number" }),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
...
export const materializedWithdrawLiquidityTotalsInV1Cosmos = v1Cosmos
  .materializedView("materialized_withdraw_liquidity_totals", {
    poolAddress: text("pool_address"),
    token0Denom: text("token0_denom"),
...
export const materializedWithdrawLiquidityInV1Cosmos = v1Cosmos
  .materializedView("materialized_withdraw_liquidity", {
    sender: text(),
    receiver: text(),
    token0Denom: text("token0_denom"),
    token0Amount: numeric("token0_amount"),
...
export const materializedAddLiquidityTotalsInV1Cosmos = v1Cosmos
  .materializedView("materialized_add_liquidity_totals", {
    poolAddress: text("pool_address"),
    token0Denom: text("token0_denom"),
...
export const materializedStakeLiquidityInV1Cosmos = v1Cosmos
  .materializedView("materialized_stake_liquidity", {
    sender: text(),
    owner: text(),
    amount: numeric(),
    lpToken: text("lp_token"),
...
export const materializedSwapTotalsInV1Cosmos = v1Cosmos
  .materializedView("materialized_swap_totals", {
    poolAddress: text("pool_address"),
