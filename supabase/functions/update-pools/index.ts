...
Deno.serve(async (req) => {
  try {
    ...
    const newPools = allPools?.filter(
      (pool) => !existingContracts?.some((contract) => contract.address === pool.pool_address),
    );
    ...
    if (newPools && newPools.length > 0) {
      ...
      for (const pool of newPools) {
        ...
        if (!data.success) {
          console.error(
            `Function error for pool ${pool.pool_address}:`,
            JSON.stringify(data.error),
          );
          throw new Error(
            `Function error for pool ${pool.pool_address}: ${JSON.stringify(data.error)}`,
          );
        }
      }
